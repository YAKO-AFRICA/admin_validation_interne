	...
	<script>
		$(document).ready(function() {
			...
			function updateFormFields() {
				...
				} else if (typeCompte === "interimSSS" && profil === "agent") {
					cibleHtml = `
						<label for="codeagent" style="color: #000000;">
						code agent <strong style="color: #F9B233;">*</strong>
						</label>
			...
			$("#AjouterRDV").on("change", "#selectUtilisateur", function() {
				...
				console.log("utilisateur change", datas);
			});
		...
		function getSelectUtilisateurRDV(idVilleEff = "") {
			...
			$.ajax({
				...
				success: function(response) {
					...
					const selectHtml = `
						<label for="selectUtilisateur" style="color: #000000;">
						Selectionnez l'utilisateur <strong style="color: #F9B233;">*</strong>
						</label>
						<select id="selectUtilisateur" name="selectUtilisateur" class="form-control" required>
						${optionsHtml}
						</select>
					`;
